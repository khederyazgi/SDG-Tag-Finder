...
public class IndicatorController {
  ...
  @CrossOrigin
  @GetMapping("/targets/{targetId}/indicators")
  public ResponseEntity<List<Indicator>> getAllIndicatorsByTargetId(@PathVariable(value = "targetId") Long targetId) throws Exception {
    if (!targetRepository.existsById(targetId)) {
      throw new Exception("Not found Indicator with id = " + targetId);
    }

    List<Indicator> indicator = indicatorRepository.findByTargetId(targetId);
